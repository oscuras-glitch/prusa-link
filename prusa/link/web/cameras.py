...
from prusa.connect.printer.cameras import CapabilityType, CameraConfigurator, \
    CameraController, CameraStatus, ConfigError, AlreadyExistsError, \
    ResolutionCapability, NotSupported, CameraNotDetected
...
@check_api_digest
def set_order(req):
    """Get the working camera settings"""
    camera_configurator: CameraConfigurator
    camera_configurator = app.daemon.prusa_link.camera_configurator
...
def get_camera(_, camera_id):
    """Write out the specified camera's config"""
    camera_configurator: CameraConfigurator
    camera_configurator = app.daemon.prusa_link.camera_configurator
    camera_controller: CameraController
    camera_controller = app.daemon.prusa_link.printer.camera_controller
    ...
    camera = camera_controller.get_camera(camera_id)
    settings = camera.get_json_settings()
    if CapabilityType.RESOLUTION in camera.supported_capabilities:
        settings["available_resolutions"] = [
                ResolutionCapability.to_json(res)
                for res in camera.available_resolutions
            ]
    settings["name"] = camera.name
    string_caps = map(lambda i: i.value, camera.supported_capabilities)
    settings["supported_capabilities"] = list(string_caps)
...
def set_settings(req, camera_id):
    ...
    settings = camera.convert_settings_from_json(json_settings)
    camera.set_settings(settings)

    if "name" in json_settings:
        camera.name = json_settings["name"]
